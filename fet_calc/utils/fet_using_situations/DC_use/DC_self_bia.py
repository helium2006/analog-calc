'''计算直流自给偏置场景下的FET漏极电流I_D_DC_self_bia，并将结果存入params字典'''
...
def calc_I_D_DC_self_bia_without_lambda(params:dict):
    """不考虑沟道调制效应时计算直流自给偏置下的漏极电流"""
    ...
    if valid_root is not None:
        params['I_D_DC_self_bia']=valid_root  # 将有效根存入params的ID
    else:
        params['I_D_DC_self_bia']=None  # 无有效根时，ID设为None
    return params

def calc_I_D_DC_self_bia_with_lambda(params:dict):
    """考虑沟道调制效应时计算直流自给偏置下的漏极电流"""
    # 首先获取不考虑沟道调制效应时的解
    params = calc_I_D_DC_self_bia_without_lambda(params.copy())
    ID_without_lambda = params.get('I_D_DC_self_bia')
    ...
    if ID_without_lambda is not None and ID_without_lambda > 0:
        ...
        if λ > 0:
            ...
            params['I_D_DC_self_bia'] = ID_with_lambda
            # 存储原始值以便比较
            params['I_D_DC_self_bia_without_lambda'] = ID_without_lambda
    
    return params

def calc_I_D_DC_self_bia(params:dict):
    """根据是否存在λ参数选择合适的计算函数"""
    λ=params.get('λ',0)  # 默认为0
    if not λ:  # 不考虑沟道调制效应
        return calc_I_D_DC_self_bia_without_lambda(params)
    else:  # 考虑沟道调制效应
        return calc_I_D_DC_self_bia_with_lambda(params)